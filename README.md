# SDYnotes 14.23.1 — Fastify + Python worker + Oracle 자체 저장소
...
- **14.23.1 논문 읽기 안정화 — 상자 사라짐·쪽 겹침·'이 페이지' 정리 수정**:
  - **겹칩 텍스트상자가 사라지던 것을 고쳤습니다.** `sanitizePageEls` 가
    "다른 상자와 88% 겹친다"는 이유만으로 상자를 지우고 문서 데이터까지
    갱신해, 사용자가 겹쳐 놓은 캡션·메모·번역 상자가 누르거나 재렌더되는
    순간 사라졌습니다. 이제 **겹침 + 글 내용이 사실상 같은**(가져오기 이중
    추출) 경우에만 중복으로 지웁니다.
  - **여러 쪽의 내용이 한 쪽에 겹쳐 쌓이던 것을 고쳤습니다.** 동기 op 의 쪽
    번호가 문서 범위를 벗어나면 마지막 쪽에 끼워 넣던(`Math.min`) 것을,
    **범위 밖 op 는 무시**하도록 바꿨습니다.
  - **해돌이 '이 페이지' 정리가 한 쪽분량답게 나옵니다.** 요청에 범위
    (`scope:page`) 를 실어 서버 프롬프트가 "지금 본문은 보고 있는 한 쪽"임을
    알게 하고, 캐시 키도 범위별로 갈랐습니다. 먼 쪽 본문을 아직 불러오는
    중이면 "없다"가 아니라 "불러오는 중"이라고 말합니다. 질문칸은 화면 맨
    아래에서 22px 위로 올렸습니다(말풍선·기록도 같이).
  - **코드 고치는 원칙을 문서로 남겼습니다** — 틀린 줄은 지우고 그 자리에
    정확한 코드를 쓰는 방식(줄 불리기·보정 코드 금지): `docs/fix_discipline.md`.
  - 회귀 테스트: `npm run test:aiPage` (해돌이 페이지 범위 · 가져온 논문
    lazy 슬라이스 런타임 2종, 각 ~7초).
