# SDYnotes 14.44.0 — Fastify + Python worker + Oracle 자체 저장소

- **14.44.0 고정 위치(논문) 편집 진입 — 단어 간격이 미세하게 변하며 마지막 단어가 다음 줄로 넘어가 겹치는 것**:
  - **문제 (보고: "편집하려 들어가면 위치는 안 변하는데 단어와 단어 사이 간격이
    미세하게 변하며 텍스트가 상자 밖으로 나가는 판정이라 다음 줄로 마지막 단어가
    넘어가서 글자가 겹침").** 읽기 모드는 단어마다 `transform:scaleX(PDF진행폭 ÷
    자연폭)` 을 씌워 원문 진폭에 맞춘다. 편집 진입의 줄 흐름 변환은
    ① 배율이 **이미** 반영된 폭에 배율을 한 번 더 곱해 간격을 재고(→ 간격이
    `(1−배율)×단어폭` 만큼씩 부풀고), ② 단어 span 에 배율을 넘기지 않아 글자가
    브라우저 **자연 폭**으로 그려진다(→ 줄이 그만큼 더 길어진다). 둘이 같은 방향으로
    겹쳐 한 줄이 상자보다 길어지고(= 밖으로 나가는 판정), 줄 높이는 '다음 줄까지'로
    박혀 있어 넘친 마지막 단어가 아래 원문 줄에 그대로 겹쳤다.
  - **고친 것 (`_tightToLineFlow`).** '화면에 보이던 진행 폭'을 그대로 옮긴다.
    - 진행 폭(adv)은 배율을 **한 번만** 반영한 값 → 간격 계산이 원문 left 차이 그
      자체가 되고, 줄의 오른쪽 끝이 원문과 같은 자리에 온다.
    - 흐름에서 단어가 차지하는 폭(자연 폭)을 실측(rect → offsetWidth → ㉠÷배율)해
      `margin-right = PDF진행폭 − 자연폭` 으로 보정한다 → **흐름 진행 폭 = 원문 진행
      폭**. `transform` 은 레이아웃에 영향이 없으므로 이 보정이 있어야 각 단어가
      자기 원문 left 에 앉는다. 치자 폭은 고정하지 않는다 — 입력하면 뒤 단어가
      밀려나는 일반 텍스트 흐름(14.40 설계)은 그대로다.
    - `scaleX` 배율도 같은 값으로 넘긴다 → 글자 모양까지 진입 전 화면과 같다.
      (맞춤이 아직 안 돌았어도 `data-pdf-w` 로 스스로 배율을 구한다.)
    - 첫 단어의 left(들여쓰기·중앙 정렬)를 `padding-left` 로 살리고 줄에
      `text-align:left` 를 박는다 — `left:0` 에 붙여 줄 전체를 왼쪽으로 당기지 않는다.
    - 줄은 `white-space:nowrap` 로 한 줄로 남긴다. 세로가 박힌 구조에서 줄이
      넘치면 아래 줄과 겹치기만 하므로, 넘치는 글자는 오른쪽으로 흐르게 한다.
  - **영향 범위.** `07b-hl-band.js`(줄 흐름 변환 · 엔터가 만드는 새 줄)와
    `sdynotes.css`(`.sdy-tl`)만 고쳤다. 원문 좌표·줄 묶음 판정·형광펜·엔터·저장
    경로는 건드리지 않았다.
  - **검증.** `npm run test:tight` **PASS 177 / FAIL 0** (신설 ⑭ 15개 — 배율가 걸린
    원본에서 '흐름 진행 폭 = PDF 진행 폭', 스페이서 = 원문 갭, 줄 오른쪽 끝 ≤ 상자 폭,
    들여쓰기·nowrap 보존, 저장·재진입 동일 배치). 회귀: typeflow 56 · textbox 53 ·
    format 65+18+41 · txtstyle 90 · selux 53 · uflow 42 · hlband 26+15 · table 38+22 ·
    font 18 · undo 18+36 · dblcaret 17 · txtedge 17 · txtclip 18 · activation 57 ·
    editlag 53+20 · export 44+28 · place 20 · fastops 9 · imgsel 35+27 · openfast 33 ·
    virtual 38+19+29 · `bundle:check` 양호.
