...
- **14.15.3 암기카드 다듬기**: ① 전체화면 홈의 '단축키 안내/스마트 학습' 팁 문구를
  제거했다. ② 학습 방식 버튼(빠르게 풀기·마스터 학습 등)에서 색을 걷어내 무채색
  글래스로 통일하고(빨주노초 6색 타일 제거), 접힌 창에서는 버튼이 영역 안에
  일정한 간격으로 꽉 차게 3열 그리드로 배치한다 — 집중 타이머(#focusClock)의
  `.fc-modes` 규칙이 암기카드 창까지 새어 나와 한 줄 flex 로 뭉개져 오른쪽이
  비던 문제를 `#focusClock` 한정으로 고쳤다. ③ 해달은 14.15.2 의 진짜 해달 복원을
  유지한다.
- **14.15.2 암기카드 해달 복원**: 14.15.1 에서 바뀐 납작 해달은 눈이 점처럼 작고
  -15° 기울어진 자세라 "예쁜데 눈이 없고 깨져 보인다"는 지적을 받았다. 마지막
  Gemini 대화의 **진짜 해달**(14.15.0)을 홈 히어로·학습 스테이지 양쪽에 그대로
  되돌렸다: 하이라이트 3개가 반짝이는 크고 까만 눈, 긴 흰 수염, 크고 까만 코,
  광택 그라데이션 갈색 털, 물결·그림자까지. 표정 연동(idle/happy/sad/think/love/wow)·
  깜빡임·머리/귀/꼬리 미세 모션·말풍선·파티클은 그대로 두고, 14.15.1 의
  글래스모피즘 UI 도 유지한다. `prefers-reduced-motion` 에서 전부 쉬게 한다.
