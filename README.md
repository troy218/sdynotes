# SDYnotes 14.24.0 — Fastify + Python worker + Oracle 자체 저장소
...
- **14.24.0 해돌이 문서 편집 — `!` 한 줄로 배치·내용을 바로 고친다**:
  - 해돌이 검색창에서 `!제목을 맨 위로 옮겨 줘`처럼 **느낌표로 시작**해 Enter를
    누르면 일반 질문(`chat`)이 아니라 문서 편집(`edit`)으로 동작합니다.
    `/편집 ...`, `/edit ...`, `편집: ...`도 같습니다. 입력하는 동안 보라색
    **편집 딱지**가 떠서 실행 전부터 모드를 알 수 있습니다.
