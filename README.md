# SDYnotes 14.27.0 — Fastify + Python worker + Oracle 자체 저장소
...
- **14.27.0 해돌이 편집이 사람 손처럼 — 형광펜·표 삭제·보기 좋은 배치**:
  ...
  - **여러 대화를 묶어 문맥으로.** 예전엔 직전 편집 1턴만 실었지만, 이제
    ...
    `AI_MAX_CONTEXT`, 기본 6000자). "아까 그 상자", "방금 만든 표 지워" 같은
    이어서 하기가 이어지고, 요청 한도도 600→1200자, 출력 토큰 기본값도
    1800→2500으로 늘었습니다. 한 번에 적용하는 편집 명령은 30→40개입니다.
