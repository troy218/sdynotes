...
├─ plans.mjs             요금제(무료·3,000원·월 14,900원)와 가져오기 정책의 한 곳
...
│   ├─ verify.mjs          구조 검사 96항목 (+ 제출 전 자리표시자 경고)
...
│   ├─ test-local-docs.mjs      서버 → 기기 이관 · 클라우드 보관 · 삭제 거절 (37항목)
│   ├─ test-all.mjs        위 6단계를 한 번에
│   └─ serve.mjs           미리 보기 서버
...
```bash
node scripts/test-all.mjs         # 조립 + 검사 6단계 (권장)
node scripts/build.mjs            # 조립만
node scripts/verify.mjs           # 구조 검사 96항목
node scripts/twa-prepare.mjs --print   # TWA 준비 상태 보기
PORT=5173 node scripts/serve.mjs  # 미리 보기
```

검사는 여섯 단계다. ① 조립 ② 구조(앱 셸·아이콘·이름표·심사 요건·원본 불변·논문 로컬화)
...
⑤는 '크기가 안 맞는데 지워 버리는' 사고를, ⑥은 '결제 회원의 원본을 지워 버리는'
사고를 막는 검사다(둘 다 실제로 났던 종류의 실수다).

## 요금제에 따라 두 갈래 (17.0)

**컴퓨터 = 읽고 편집하는 곳, 패드 = 필기하는 곳.** 이 흐름을 만들려고 요금제를 올렸다.

| | 무료 (기기 전용) | 프리미엄 (월 14,900원 · 클라우드 200GB) |
|---|---|---|
| 논문 | 월 5편 | 편수 제한 없음 |
| 보관 | **내 기기** — 서버는 변환만 하고 지운다 | **내 클라우드** — 서버가 원본 |
| 컴퓨터 | 같은 기기에서만 | 어디서나 읽고 편집 |
| 패드 | 기기 안에서 필기 | 클라우드로 자동 맞춤 |

값은 두 곳에 나뉘어 있다(어긋나면 팔아 놓고 막는 사고가 난다):

| | 역할 | 파일 |
|---|---|---|
| 보여 주는 값 | 스토어·앱 안내 | `plans.mjs` |
| **강제하는 값** | 서버가 실제로 막는 상한 | `server/src/lib/plans.js` |

`verify.mjs` 가 두 값을 대조한다. 결제는 **스토어 + 웹 둘 다**
(컴퓨터로 읽는 사람이 많으니 컴퓨터에서 결제하는 길을 둔다).

앱은 켜질 때 `/api/auth/storage` 에 요금제를 물어보고, 답에 따라 갈래를 정한다 —
클라우드면 기기 사본은 오프라인용이고 **서버 사본을 지우지 않는다**. 이 판단이 틀리면
결제한 회원의 원본이 사라지므로 `test-local-docs.mjs` 가 그 경우를 따로 검사한다.

## 무료 등급: 논문은 서버에 남지 않는다
