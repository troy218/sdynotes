...
import {
  ...
  AI_RATE_N, AI_RATE_WINDOW_MS, AI_COOLDOWN_MS,
} from '../lib/config.js';
  ...
  app.post('/api/ai/ask', async (req, reply) => {
    ...
    if (!spec) {
      return reply.code(400).send({
        ok: false, error: '할 수 없는 일이에요',
        allowed: Object.keys(AI_TASKS),
      });
    }
    if (!AI_READY) return reply.code(503).send({ ok: false, error: DISABLED });

    let text = normText(b.text);
    const question = String(b.question == null ? '' : b.question).trim().slice(0, AI_MAX_QUESTION);
    if (spec.needText && !text) return reply.code(400).send({ ok: false, error: '빈 노트예요 · 먼저 노트에 글을 적어 주세요' });
    if (spec.needQuestion && !question) return reply.code(400).send({ ok: false, error: '질문을 적어 주세요' });
    if (task === 'free' && !question) return reply.code(400).send({ ok: false, error: '질문을 적어 주세요' });

    const truncated = text.length > AI_MAX_TEXT;
    if (truncated) text = text.slice(0, AI_MAX_TEXT);
    ...
    if (!rl.ok) {
      return reply.code(429).send({
        ...
        error: `잠시 뒤에 다시 시도해 주세요 · ${AI_RATE_N}번/${Math.round(AI_RATE_WINDOW_MS / 1000)}초`,
      });
    }

    if (!anyProviderReady()) {
      // 모든 공급사가 쉬는 중. 그래도 캐시에 있으면 그걸로 답한다(외부 호출 0번).
      const hit = aiCacheGet(aiKeyFor(task, text, question));
      if (hit) {
        return reply.send({
          ok: true, task, text: hit.text, model: hit.model || AI_MODEL,
          provider: hit.provider || '', cached: true, truncated, chars: text.length,
        });
      }
      return reply.code(429).send({
    ...
    try {
      const { text: out, cached, provider, model } = await aiCore(task, text, question);
      return reply.send({
        ok: true, task, text: out, model: model || AI_MODEL, provider: provider || '',
        cached, truncated, chars: text.length,
      });
    } catch (e) {
      const limited = Boolean(e && e.limited);
      const retry = limited ? Number((e && e.retryAfterSec) || earliestCoolSec()) : 0;
      req.log?.error?.({ err: e, task, tried: e && e.tried }, 'ai ask failed');
      return reply.code(limited ? 429 : 502).send({
        ok: false,
        limited,
        retry_after: retry,
        error: limited
