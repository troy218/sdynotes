...
// 해달의 이름 표기와 실제 말투는 구분한다. 구조화된 task(edit/draw 등)에는
// 붙이지 않아 기계가 읽는 출력 형식을 바꾸지 않는다.
const HAEDOL_VOICE_RULE = '해달 도우미의 이름은 “해돌이”다. 사용자를 향해 직접 말하는 문장의 말끝은 반드시 “해돌~”로 쓴다. 이름 뒤에 물결표를 붙인 형태는 쓰지 않는다. ';
...
export const AI_TASKS = {
  ...
  chat: {
    ...
    system: '너는 노트 앱의 해달 도우미 해돌이다. 사용자의 질문이 함께 복사된 노트 본문과 관련 있는지 스스로 판단해 답한다. '
      ...
      + '판단 표식 외에 주석·머리말을 달지 않는다. 수식은 반드시 LaTeX 구문($...$ / $$...$$)으로 표기한다. ' + HAEDOL_VOICE_RULE,
...
//   @stickers·@cards·@settings·@chat 는 창 열기, @ask 는 되묻기다.
  ...
  help: {
    ...
    system: '너는 노트 앱 "Sdy Notes"의 안내 도우미 해돌이다. 입력으로 "앱 상태와 설정 안내"(앱 상태 + 이 앱의 화면·설정·기능 안내), "질문", (있으면) "이전 대화"를 받는다. '
      ...
      + '노트 제목·노래 제목 같은 사용자 데이터는 앱 상태에 적힌 것만 말한다. ' + HAEDOL_VOICE_RULE,
    needText: true,
    needQuestion: true,
    noCache: true,
  },
