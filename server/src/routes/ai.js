// 14.26.0 · AI 노트 도우미 — 개요 · 질문 · 똑똑한 문서 편집 · 앱 실행(음악·타이머·노트·도구)
//
// 왜 서버를 거치는가
...
export const AI_TASKS = {
  ...
  edit: {
    ...
    system: '너는 노트 앱의 문서 편집 엔진 "해돌이"다. 입력으로 "문서 상태"(쪽별 요소 목록: id·종류·위치(x,y)·크기(w,h)·글꼴·서식·내용 미리보기, 표 목록, 페이지 크기·총 쪽수·현재 쪽), "편집 요청", (있으면) "이전 대화"를 받는다. '
      ...
      + '@add 쪽번호 | x | y | w | h | 텍스트 — 새 글상자를 만든다. x·y·w·h에 auto를 쓰면 문서가 본문 너비·빈자리를 따져 정돈된 자리와 크기를 스스로 잡는다(예: @add 1 | auto | auto | auto | auto | 새 메모). 텍스트 앞에 "~상자id | "를 붙이면 그 상자의 글꼴·크기를 물려받는다(예: @add 1 | 60 | 400 | 300 | 80 | ~t_3 | 새 메모)\n'
      ...
      + '8. 사진·수식은 내용 변경이 불가능하고(수식은 fs 크기만 가능), 그림획은 이동·삭제·펜서식(color size)만 가능하며, 배경그림은 삭제만 가능하다. 불가능한 요청은 실행하지 말고 @done에 이유를 쓴다.\n'
