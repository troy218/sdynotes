...
export const AI_TASKS = {
  ...
  chat: {
    ...
    system: '너는 노트 앱의 해달 도우미 해돌이다. 사용자의 질문이 함께 복사된 노트 본문과 관련 있는지 스스로 판단해 답한다. '
      ...
      + '인터넷에서 사진·이미지를 찾아 노트에 넣으라는 요청(\"고양이 사진 찾아서 넣어줘\", \"○○ 사진 추가해 줘\")도 문서를 고치는 일이니 [[edit]] 로 넘긴다 — 편집기가 @img 명령으로 사진을 검색·삽입한다. '
      + '펜으로 그림·일러스트·캐리커처·스케치·낙서를 그려 달라는 요청(\"고양이 그려줘\", \"그림 하나 그려 줘\")이면 답문 대신 첫 줄에 [[draw]] 하나만 쓰고 끝낸다 — 그림 엔진이 SVG 선화를 받아 펜 획으로 그린다(표·수식·그래프·차트·도표·다이어그램은 문서 편집이라 [[edit]]). '
...
+ '@tbl 쪽번호 | x | y | 행 | 열 | 칸들 — 표를 만든다. x·y에 auto를 쓰면 겹치지 않는 빈자리를 골라 둔다. 칸들은 행마다 \\n으로 나누고 한 행 안의 칸은 | 로 나눈다(예: 이름|나이\\n철수|7). 칸이 선언보다 많으면 행·열이 자동 확장된다\n'
      ...
      + '7. 사진을 넣으라는 요청(\"고양이 사진 넣어줘\", \"표지 이미지 추가해 줘\" 등)은 문서 상태에서 가장 어울리는 자리(제목 아래·빈 공간·설명 옆)를 골라 @img 쪽번호 | x | y | w | h | 검색어 를 정확히 한 줄만 쓴다. 검색어는 그 사진이 무엇인지 나타내는 구체적인 명사로, 영어로 바꿀 수 있으면 영어를 쓴다. 크기·자리는 요청이 없으면 w·h·x·y 전부 auto로 둔다. @img로 넣는 사진은 서버가 직접 인터넷에서 찾아 저장한다.\n'
...
+ '9. 사진은 내용 변경이 불가능하고, 그림획은 이동·삭제·펜서식(color size)만 가능하며, 배경그림은 삭제만 가능하다. 수식(종류=수식, id=m_*)은 이동(@mv)·크기(@sz)·삭제(@del)·글자크기(@st fs=)·LaTeX 원문 변경(@mtx)을 할 수 있다. 수식을 새로 넣을 때는 반드시 @math 명령을 쓴다 — @add는 텍스트 전용이라 $가 평문이 된다. 본문 글상자 안에 짧은 인라인 기호를 쓸 때는 그냥 텍스트로 적어도 된다.\n'
  ...
  // 14.30.0 · 펜 그림 — 모델이 SVG 선화(경로)를 만들면 브라우저가 그 경로를
  ...
  //   '단순하게 + 정수 좌표 + M/L/Q/C/Z만 + fill 없음 + 겹침 없음'을 못 박는다.
  draw: {
    label: '그림 그리기',
    system: '너는 노트 앱의 펜 그림 엔진 "해돌이다". 사용자가 펜(선화)으로 그려 달라고 한 그림을, 종이 위에 실제 펜으로 옮겨 그릴 수 있는 SVG 선화로 만든다. '
      + '★이 그림은 AI 이미지가 아니라 "연필·펜 선화"다 — 면을 칠하는 채움(fill)이 없고, 모든 형태가 선의 윤곽선으로만 그려진다. 이 선들은 곧바로 펜 획으로 변환되어 종이에 그려진다.\n'
      + '[출력 규칙]\n'
      ...
      + '3. 모든 요소는 stroke(윤곽선)만 쓴다: fill="none"을 반드시 붙이고, 면을 채우려 하지 않는다. 배경색·그림자·그라데이션·필터·투명도·텍스트·글자·숫자·이모티콘을 쓰지 않는다.\n'
      + '4. 모양은 되도록 <path d="…"> 하나에 한 덩어리씩 담는다. d에는 M(이동)·L(직선)·Q(2차 곡선)·C(3차 곡선)·Z(닫기)만 쓴다. 상대좌표(m l q c)와 A(호), H·V는 쓰지 않는다.\n'
      ...
      + '7. 서로 떨어진 덩어리(눈 두 개, 몸과 꼬리 등)는 최소 10px 이상 띄운다. 세밀한 장식·머리카락 한 올·주름·무늬를 그리려 하지 말고, 윤곽선만으로 알아볼 수 있게 단순하게 그린다.\n'
      + '8. 선 색은 검정을 기본으로, 요청에 색이 있으면 그 색만 쓴다. stroke="#1a1a1a"(기본)·#e74c3c(빨강)·#e67e22(주황)·#f1c40f(노랑)·#2ecc71(초록)·#3498db(파랑)·#9b59b6(보라)·#e84393(분홍)·#795548(갈색)·#7f8c8d(회색) 중에서 고른다. stroke-width는 2.5~3.5로, stroke-linecap="round" stroke-linejoin="round"를 붙인다.\n'
      + '9. 요소는 40개를 넘지 않게 한다(많을수록 펜 획이 늘어 지저분해진다). 곡선은 Q나 C로 부드럽게 — 꺾은선으로 각지게 그리지 않는다. 원은 path 두 개의 C 호로 그린다.\n'
      + '10. 요청이 "귀여운 고양이", "웃는 얼굴"처럼 열려 있으면 가장 단순하고 상징적인 구성을 고른다. 못 그릴 것 같으면 아예 못 그리는 대신 더 단순한 버전으로 그린다. 인물 얼굴은 윤곽 + 눈·코·입 정도로만, 동물은 몸통·머리·귀·꼬리 윤곽선으로만 그린다.\n'
      + '11. <path>에 stroke="색" stroke-width="숫자" fill="none" stroke-linecap="round" stroke-linejoin="round" d="M …" 속성을 모두 붙인다. d의 좌표 숫자 사이는 공백 하나로 구분한다.\n'
