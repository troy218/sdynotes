...
/* APP-PART:11a-text-style.js:BEGIN */
    ...
    function withSelection(fn){
        ...
        saveSel();
        syncCurSel();
        // 14.45.1 · restoreSel 은 서식을 입히기 위해 host 를 '임시로' 편집 가능하게 연다.
        //   편집 중인 상자가 아니라면 반드시 되돌려야 한다 — 안 그러면 미리보기 상자가
        //   캐럿이 깜빡이는 편집 상태로 남는다(형광펜·글자색을 미리보기에서 골라
        //   칠할 때의 경로). 선택 저장을 마친 뒤 가장 마지막에 닫는 이유는, 닫는
        //   순간 라이브 선택이 접힐 수 있어 저장된 범위가 먼저 갱신돼야 다음 커맨드가
        //   같은 범위를 되살릴 수 있기 때문이다. 편집 상자는 enterEdit/퇴장이
        //   소유권이므로 건드리지 않는다.
        try{ if(!w||!w.classList.contains('edit')) host.contentEditable='false'; }catch(_e){}
        return true;
    }
