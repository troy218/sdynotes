...
/* APP-PART:09-tools-ui.js:BEGIN */
    ...
    // ---------- ⑩ 자동 백업 내려받기 ----------
    // ==========================================================
    //  단어 빈도 분석 — 중요어 색칠 (Task 31 · 14.39.6 에서 '산만함' 수정)
    //  문서의 핵심어를 한눈에 알아볼 수 있게 물들인다.
    //  (화면 표시 전용 · 원문 변경 없음)
    //
    //  14.39.6 · 사용자 보고 "너무 산만하다. 진짜 중요한 단어만".
    ...
    const WF_STOP=new Set([
        ...
        // 14.39.6 · 뜻이 옅어 칠해도 도움이 안 되던 일반 명사·부사를 더 걸러 낸다
    ...
    const WF_VERB=['하였습니다','되었습니다','하겠습니다','되겠습니다','했습니다','합니다','됩니다',
                   ...
                   '하여','시켜','시킨',
                   // 14.39.6 · 한 글자 어미도 뗀다 (기록해 → 기록 · 제한된 → 제한 · 가능할 → 가능)
    ...
    function wfStem(w){
        ...
        // 14.39.6 · 조사는 '한 번만' 뗀다. 두 번 떼면 '캘빈회로가 → 캘빈회로 → 캘빈회'
    ...
    // 14.39.6 · 용언(동사·형용사)의 활용형은 '중요어'가 아니다.
    ...
    // 14.39.6 · 끝 글자가 조사처럼 보여 한 낱말이 둘로 쪼개지는 일을 막는다.
    ...
    function wfPaintNode(root){
        ...
        texts.forEach(tn=>{
            ...
            // 14.39.6 · '기준 횟수를 넘은 모든 낱말'이 아니라 '고른 중요어'만 칠한다
