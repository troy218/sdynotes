# AI 노트 도우미 설정 (14.24.0 · 해돌이 문서 편집)

**14.24.0부터 해돌이는 답만 하지 않고 문서도 직접 고친다.** 검색창에
`!제목을 맨 위로 옮겨 줘`처럼 **`!`로 시작해 Enter**를 누르면 문서 편집 모드다.
`/편집 ...`, `/edit ...`, `편집: ...`도 같은 뜻이다. 입력 중에는 검색창에
...
반영한다. 모든 변경은 **Ctrl+Z 한 번으로 되돌릴 수 있다**.
...
1. 프런트의 `capture()`가 현재 노트 revision과 구조 스냅샷을 잡는다. 스냅샷에는
   페이지 크기/번호와 요소 id·종류·좌표·크기, 최대 240자의 평문 미리보기만 들어간다.
2. 서버 `task=edit` 프롬프트는 모델이 `@mv`, `@sz`, `@bx`, `@tx`, `@add`,
   `@del`, `@done`만 한 줄씩 내도록 제한한다. 문서 본문 안의 가짜 프롬프트는
   사용자 데이터로 취급하고 따르지 말라는 규칙도 함께 보낸다.
3. 프런트는 스트림 중 명령을 숨기고 **완료 뒤에만** 파싱한다. 알 수 없는 명령,
   잘못된 숫자, 없는 id, 잠긴 요소, 표 칸, 허용되지 않는 요소 작업은 건너뛴다.
   모델 텍스트는 HTML을 이스케이프한 평문으로만 저장한다.
4. 적용 직전에 revision을 다시 비교한다. 그 사이 다른 기기/사용자가 문서를 바꿨다면
   오래된 계획은 하나도 실행하지 않고 다시 요청하라고 알린다.
5. 성공하면 변경한 페이지를 다시 그리고 저장·동기화한다. 최대 30개 명령을 하나의
   undo 스냅샷으로 묶으므로 Ctrl+Z 한 번이면 요청 전 상태로 돌아간다.
...
- **키는 서버에만** — 프런트에는 없다.
- **task 화이트리스트** — `outline/chat/edit` 3종뿐. 임의 system 프롬프트
  주입 불가(그 외 값은 400).
- **편집 허용 목록 + revision 검사** — 모델은 문서 객체를 직접 받거나 실행하지
  못한다. 검증된 6가지 변경만 가능하고, 요청 중 문서가 달라지면 전부 취소한다.
- **캐시 + in-flight 합치기** — 같은 개요/질문은 모델 호출을 한 번으로 줄인다.
  **`edit`은 중복 추가 방지를 위해 예외로 매번 새 계획을 만든다.**
...
- `test/ai_frontend_contract.mjs` — 소스 계약 + jsdom에서 질문/편집을 실제로 보내
  **`!` 모드 표시 · 스냅샷 전송 · 스트림 중 부분 적용 금지 · 허용 명령 파싱 · stale
  안내**까지 검사한다.
- `test/ai_edit_runtime.mjs` — 실제 서버와 편집기를 열어 스냅샷·좌표 제한·잠금/표
  거절·HTML 이스케이프·저장 반영·**Ctrl+Z 전체 원복·revision 충돌 취소**를 검증한다.
