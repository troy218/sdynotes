# AI 노트 도우미 설정 (14.37.0 · 참고 일러스트를 따라 그리는 해돌이 · 사진/그림 구분)
...
- **출처 표기.** 해돌이 말풍선에 `(참고: OpenMoji · CC BY-SA 4.0)` 을 붙인다. 번들의
  저작권 안내는 `server/assets/haedol_refs.LICENSE.txt` (OpenMoji CC BY-SA 4.0 ·
  CLDR 한국어 주석 Unicode-3.0).
- **검증.** `npm run test:refdraw` — 검색·SVG·API 계약 57개(`test/ai_refdraw_contract.mjs`)
  + 실제 편집기 런타임 38개(`test/ai_draw_photo_runtime.mjs` ⑤: 진짜 번들로 고양이를
