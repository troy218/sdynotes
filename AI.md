# AI 노트 도우미 설정 (14.34.0 · 참고 일러스트를 따라 그리는 해돌이)

**14.34.0 — "그림 그려 줘"는 이제 모델이 맨손으로 그리지 않고, 참고 일러스트의 윤곽을 따라 그린다.**
- **왜.** 14.31.0 에서 화풍·디테일 지시를 아무리 늘려도 모델이 좌표를 맨손으로 찍는
  그림은 알아보기 어려운 것이 많았다. 그래서 순서를 바꿨다 — **먼저 부탁한 대상의
  참고 일러스트(선화)를 찾고, 그 윤곽을 펜 획으로 따라 그린다.** 참고 그림이 있으면
  결과 품질은 보장되고, 모델은 아예 부르지 않아 더 빠르다(무료 한도도 아낀다).
...
- **참고 그림이 없을 때만 예전 길.** 번들에 없는 것(`zzqq …`)은 `404 {reason:'nomatch'}`
  → 클라이언트가 예전처럼 모델 직접 그리기(`task=draw`, 14.31.0 화풍 규칙)로 내려간다.
  네트워크 오류·429 도 마찬가지로 조용히 내려간다.
- **획 품질.** 참고 선화를 펜으로 옮길 때 긴 직선은 3.5px 간격으로 점을 채워 종이 위
  곡선 보간에도 집·상자의 모서리가 뭉개지지 않게 했고, 이어 붙인 획이 색·굵기를 잃던
  버그도 고쳤다.
...
- **그림이 꼼꼼해졌다.** 화풍을 *귀엽고 단순한 일러스트*(스티커·동화·카툰 느낌:
