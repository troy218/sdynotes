# AI 노트 도우미 설정 (14.28.2 · 수식(@math)·논문 최적화·제목 자동 확대·중요어 굵게)

**14.28.2 — 제목은 크고 굵게, 중요어는 굵게.**
- 해돌이가 글을 쓸 때 제목(첫 줄·장/절 시작)인지 본문인지 스스로 판단해,
  제목은 자동으로 `#`/`##`/`###` 크고 굵은 서식(30/24/20pt·bold)으로 넣고
  본문은 16pt 평문으로 분리해 준다.
- 본문 안의 핵심 용어·결론·숫자·정의는 자동으로 `**굵게**`(말풍선) 또는
  `@hl`(노랑 형광펜)/`bold=on`(문서 글상자)으로 강조한다(한 문장 1~3단어).
- 말풍선 답변도 `#`/`##`/`###` 제목과 `**굵게**`, 목록(-/1.), \`코드\`,
  $수식$을 실제 서식으로 렌더링한다(말하는 중에는 평문으로 빠르게 흘리고
  다 말한 뒤에만 마크다운을 그려 깜빡이지 않게 함).
- 14.28.1 이미지 선택 시 주변 요소가 사라지지 않게 수정.

**14.28.0 — 수식을 만들고 논문 질문에 최적화된다.**
- 새 명령 `@math`(`@ltx`·`@formula`·`@수식` 별칭)로 LaTeX 수식을 문서에 직접 넣는다.
  `x·y·w·h`에 auto를 쓰면 인라인/디스플레이 크기를 스스로 잡는다. `$$...$$`는
  한 줄 블록(display) 수식, `$...$`는 인라인 수식으로 알아서 판정한다.
- 기존 수식 내용은 `@mtx`(`@latex`)로 LaTeX 원문을 바꾸고, `@st m_id | fs=22`로
  글자 크기를 조절한다.
- 개요·질문이 **논문·리포트**로 보이면(서론·방법·결과·수식·p값 등 단서) 논문 구조대로
  정리·답변하고, 수식·기호·그리스 문자를 LaTeX($...$ / $$...$$)로 정확히 적는다.
- 새 글상자 내용 앞에 `# `/`## `/`### `를 붙이면 자동으로 대제목(30pt·굵게·가운데)/
  중제목(24pt·굵게)/소제목(20pt·굵게)로 판정해 제목을 눈에 띄게 크게·굵게 만든다.
  노트 첫 상자가 짧으면 자동 제목으로 추정한다.
...
색·형광펜·굵기·기울임·밑줄·취소선·정렬과 그림획 펜색·굵기 바꾸기(`@st`),
**수식 넣기·바꾸기(`@math`·`@mtx`)**, 표 만들기·크기·이동·칸 채우기(`@tbl`/`@tsz`/
`@tmv`/`@tcell`), 제목 자동 크기·굵게(`#`·`##`·`###`), 클립보드 붙여넣기·복사하기
...
| 변수 | 기본 | 뜻 |
|---|---|---|
| `AI_MAX_TOKENS` | `8000` | 응답 길이 상한 (256~16000 clamp). 장문 작성과 최대 120개 편집 명령을 끝까지 받는다 |
| `AI_MAX_TEXT` | `80000` | 한 번에 보내는 노트 본문/편집 상태 길이(자, 최대 20만). 넘으면 **앞 40% + 중간 30% + 뒤 30%** 를 살린다 (`truncated:true`, `note_chars` 에 원본 길이) |
| `AI_MAX_QUESTION` | `6000` | 사용자 요청 길이 상한(자, 최대 2만). 여러 상자의 상세 지시와 긴 초안을 통째로 받는다 |
| `AI_MAX_CONTEXT` | `16000` | `edit`·`app` 에 싣는 **이전 대화 문맥** 상한(자, 최대 5만). 최근 5턴(질문·편집·실행)을 이어서 싣는다 |
| `AI_PART_CHARS` | `18000` | 긴 편집 상태를 나누는 목표 크기(자). 작은 컨텍스트 모델의 과부하를 줄인다 |
| `AI_MAX_PARTS` | `4` | 긴 편집을 병렬 처리할 최대 파트 수(1~8) |
| `AI_CONTINUE_PARTS` | `4` | 모델이 출력 한도에 닿았을 때 자동으로 이어 받을 최대 파트 수(1~8) |
| `AI_TIMEOUT_MS` | `120000` | 모델 호출 타임아웃 (스트림은 '전체' 시간) |
...
2. 서버 `task=edit` 프롬프트는 모델이 `@mv`, `@sz`, `@bx`, `@tx`, `@add`,
   `@math`(`@ltx`·`@formula`), `@mtx`(`@latex`), `@del`(`@tdel`), `@rp`,
   `@ap`, `@st`, `@hl`, `@tbl`, `@tsz`, `@tmv`, `@tcell`, `@tidy`, `@goto`,
   `@newpage`, `@title`, `@clip`, `@clipin`, `@copy`, `@ask`, `@done`만
   한 줄씩 내도록 제한한다. `# `/`## `/`### `로 시작하는 @add 내용은 대·중·소제목으로
   자동 판정해 30/24/20pt·굵게로 넣는다. 문서 본문 안의 가짜 프롬프트는
   사용자 데이터로 취급하고 따르지 말라는 규칙도 함께 보낸다.
...
6. 성공하면 변경한 페이지를 다시 그리고 저장·동기화한다. 최대 120개 명령을 하나의
...
- **본문 길이 상한** — 긴 문서도 상한만큼만 보낸다. **앞 40% + 중간 30% + 뒤 30%** 를
  살리므로 '앞부분만 읽고 답하는' 일이 없다(사이 두 구간만 접는다).
...
- `test/ai_contract.mjs` — 요청 모양(키가 헤더에만)·task 화이트리스트·캐시·
  in-flight·길이 상한(앞·중간·뒤 3구간)·429 제한 안내(쿨다운 없음)·401 처리 +
...
- `test/ai_frontend_contract.mjs` — 소스 계약 + jsdom에서 질문/편집/실행을 실제로 보내
  **`!` 없이 자동 라우팅 · 편집 딱지 · 스냅샷 전송 · 스트림 중 부분 적용 금지 ·
  새 명령 파싱(@rp/@ap/@st/@hl/@math/@mtx/@tbl…/@ask) · stale 안내 · 비동기 적용 대기 ·
