...
## ★) 버그 수정 원칙 — "줄을 덧붙이지 않고, 틀린 코드를 지우고 정확한 코드로 바꾼다"

이 저장소(`sdynotes.js` 가 1.4MB 를 넘는 단일 파일)에서 모두가 지켜야 할
수정 방식이다. 새 기능이든 버그 수정이든 **코드 줄 수를 늘리는 방식으로
고치지 않는다.**

1. **틀린 코드는 삭제가 답이다.** 방어 `try/catch`, 재시도 덧댐, '혹시 몰라서'
   플래그로 덧붙이면 증상만 가려지고 원인은 남는다. 잘못된 문장은 그 자리에서
   지우고, 올바른 문장으로 **교체**한다 (덮어쓰기 — not 감싸기).
2. **같은 계산이 둘 이상이면 하나로 모은다.** 예: `onEditorScroll` 의
   스크롤→쪽 번호 계산을 `visiblePageIdx()` 하나로 빼 해돌이 bridge 도 같은
   함수를 쓰게 했다. 복사해 둔 비슷한 수식이 어긋나면 또 버그가 난다.
3. **화면(렌더)은 데이터를 건드리지 않는다.** 그리기 위해 거른 결과를 다시
   `doc` 에 덮어써 요소를 지운 사례(`renderPageEls` 의 sanitize 되덮어쓰기)가
   있었다 — 렌더는 읽기만 하고, 데이터 정리는 책임 있는 경로(가져오기·저장)에서
   한 번만 한다.
4. **빈 껍데기(lazy 스텁)를 '없는 문서'로 다루지 않는다.** 슬라이스 로딩 중인
   쪽의 `els` 는 `[]` 일 뿐 '빈 쪽'이 아니다 — 이 상태에서 DOM 을 지우면
   상자가 사라진다(`_selectiveRenderPage` 의 lazy 가드 참고).
5. **수정 후엔 빠른 계약 테스트로 못박는다.** `test/*_contract.mjs`(소스 정규식,
   1초 내) + 필요한 것만 jsdom 런타임 몇 초. 느린 통합 테스트를 늘리지 않는다.
