...
2. 서버 `task=edit` 프롬프트는 모델이 `@mv`, `@sz`, `@bx`, `@tx`, `@add`,
   `@del`, `@rp`, `@ap`, `@st`, `@tbl`, `@tsz`, `@tmv`, `@tcell`, `@goto`,
   `@newpage`, `@title`, `@clip`, `@clipin`, `@copy`, `@ask`, `@done`만 한 줄씩
   내도록 제한한다. 문서 본문 안의 가짜 프롬프트는 사용자 데이터로 취급하고
   따르지 말라는 규칙도 함께 보낸다.
...
- **편집 허용 목록 + revision 검사** — 모델은 문서 객체를 직접 받거나 실행하지
  못한다. 검증된 명령 목록(이동·크기·내용·추가·삭제·부분수정·덧붙이기·서식·표·
  클립보드·쪽·제목·되묻기)만 가능하고, 요청 중 문서가 달라지면 전부 취소한다.
...
- `test/ai_contract.mjs` — 요청 모양(키가 헤더에만)·task 화이트리스트·캐시·
  ...
  **14.25.0 edit 프롬프트·`[[edit]]`·후속 context** (73 검사)
