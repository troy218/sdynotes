...
//   2) task 화이트리스트 밖의 일은 400 으로 거절한다 (임의 프롬프트 주입 차단)
...
ok('status: 할 일 4종(summarize/bullets/ask/free)', Array.isArray(j.tasks) && j.tasks.length === 4
  && j.tasks.map((t) => t.id).join(',') === 'summarize,bullets,ask,free');
ok('status: 키가 응답에 새지 않는다', !g.body.includes(FAKE_KEY) && !g.body.includes(FAKE_KEY.slice(3)));

// ── 2) 요약: OpenAI 호환 본문으로 나가고 키는 헤더에만 ──
calls = [];
extFetch = () => chatOk('세 문장 요약입니다.');
let r = await post('/api/ai/ask', { task: 'summarize', text: '오늘 배운 것. 광합성은 엽록체에서 일어난다.' });
ok('요약 성공', r.ok === true && r.text === '세 문장 요약입니다.');
const c0 = calls[0];
...
// ── 3) 질문: 질문이 프롬프트에 실린다 ──
calls = [];
extFetch = () => chatOk('엽록체입니다.');
r = await post('/api/ai/ask', { task: 'ask', text: '광합성은 엽록체에서 일어난다.', question: '어디서 일어나?' });
ok('질문 성공', r.ok === true && r.text === '엽록체입니다.');
ok('질문이 user 메시지에 담긴다', /질문: 어디서 일어나\?/.test(calls[0].body.messages[1].content));
r = await post('/api/ai/ask', { task: 'ask', text: '본문', question: '' });
ok('질문 없이 ask → 400', r.status === 400 && /질문을 적어 주세요/.test(r.error || ''));
...
ok('화이트리스트 밖 task → 400 + 외부 호출 0번', r.status === 400 && calls.length === 0
  && Array.isArray(r.allowed) && r.allowed.includes('summarize'));
...
r = await post('/api/ai/ask', { task: 'summarize', text: '캐시 테스트 본문' });
...
r = await post('/api/ai/ask', { task: 'bullets', text: '캐시 테스트 본문' });
...
const both = await Promise.all([
  post('/api/ai/ask', { task: 'summarize', text: '동시 요청 본문' }),
  post('/api/ai/ask', { task: 'summarize', text: '동시 요청 본문' }),
]);
...
// ── 8) 빈 노트 ──
r = await post('/api/ai/ask', { task: 'summarize', text: '   \n  ' });
...
r = await post('/api/ai/ask', { task: 'summarize', text: '429 맞는 본문' });
...
r = await post('/api/ai/ask', { task: 'summarize', text: '쿨다운 중 본문' });
ok('쿨다운 중에는 모델 API 를 부르지 않는다', r.status === 429 && calls.length === 1);
await new Promise((z) => setTimeout(z, 350));   // 쿨다운 만료
r = await post('/api/ai/ask', { task: 'summarize', text: '쿨다운 뒤 본문' });
...
r = await post('/api/ai/ask', { task: 'summarize', text: '키 오류 본문' });
ok('401 → 502 (limited 아님)', r.status === 502 && r.limited === false);
r = await post('/api/ai/ask', { task: 'summarize', text: '키 오류 본문 두번째' });
ok('401 은 쿨다운이 없어서 곧바로 다시 나간다', calls.length === 2 && r.status === 502);

// ── 10-2) 설정 오류 힌트: 401 은 키, 404 는 모델명을 짚어 주고 키는 안 싣는다 ──
ai.aiCacheReset(); ai.aiCooldownReset();
extFetch = () => new Response('invalid api key', { status: 401 });
r = await post('/api/ai/ask', { task: 'summarize', text: '힌트 401 본문' });
...
r = await post('/api/ai/ask', { task: 'summarize', text: '힌트 404 본문' });
...
r = await post('/api/ai/ask', { task: 'summarize', text: '힌트 429 본문' });
