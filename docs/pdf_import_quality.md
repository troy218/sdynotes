...
## 14.46.0 — 행렬 복원 (사용자 보고: "행렬도 잘 잡게 해줘")

검증일: **2026-09-10** · 기준 버전: **14.45.0** · 결과 버전: **14.46.0**

> 물리·컴퓨터구조 논문을 자주 본다는 사용자의 보고를 받았다. **공개 물리·
> 컴퓨터구조 논문은 샌드박스에서 내려받을 수 없어(외부망 차단) 실제 논문으로는
> 검증하지 못했다 — 사용자 PDF 검증 대기.** 아래는 표본과 코드로 검증한 내용이다.

| 증상 | 원인 | 고친 방법 |
|---|---|---|
| 2×2 행렬이 `\left[ a_{11} a_{12} a_{21} a_{22} \right]` 로 한 줄로 뭉개진다 | 큰 괄호를 찾아도 안쪽을 x 순서로만 읽었다 | `_try_matrix` — 안쪽을 **격자(행·열)** 로 복원해 `pmatrix`/`bmatrix`/`Bmatrix`/`vmatrix`/`matrix`/`array` 로 조립 |
| 칸이 여섯 개를 넘으면 열 구분(`&`) 없이 세로로만 늘어놓는다 | `_row_split` 이 `\begin{aligned}` 로 찢었다 | 행렬을 `aligned` 보다 먼저 판정한다(`assemble` 깊이 0) |
| 칸 숫자가 본문 글자로 흩어지고 닫는 괄호가 마지막 줄에 붙는다(`1 0[ ]`) | '큰 수식' 씨앗이 **확장 글꼴(CMEX/txex) 글리프** 와 분수선 근처에만 반응했다. 일반 글리프를 세로로 늘려 큰 괄호를 그리는 PDF(unicode-math·Word 계열)는 씨앗이 없었다 | `_matrix_band_seeds` — 글자 배치로 후보를 만들고, **실제로 행렬로 조립되는 것만** 씨앗으로 올린다 |
| 첨가 행렬 `[A \| b]` 의 가운데 막대가 칸 글자가 된다 | 키 큰 세로 막대가 줄 가르기를 무너뜨려 한 줄로 뭉개졌다 | 막대를 격자에서 빼 두고 **열 구분선**(`\begin{array}{cc\|c}`)으로 되돌린다 |
| 줄임표 칸이 `.`·`…` 로 흩어진다 | TeX 은 `\cdots \vdots \ddots` 를 마침표 세 개로 조판한다 | 점만 든 칸을 세로·가로·대각 배치로 판정해 `\cdots`/`\vdots`/`\ddots` |
| `\ddots` 가 `\ddot{s}` 로, `\dots` 가 `\dot{s}` 로 깨진다 | `_bind_accents` 의 악센트 이름표가 `ddot` 뒤의 `s` 를 인수로 삼켰다(단어 경계 없음) | 악센트 명령 끝에 `\b` 를 붙인다 — **줄임표 전체**가 깨져 있던 문제 |
| 식 한가운데 `\int` 가 생긴다 | MuPDF 가 멀리 떨어진 두 글리프 사이에 넣는 **가짜 빈칸**을 확장 글꼴 글리프로 읽었다(CMEX 코드 32 = integral) | 확장 글꼴의 빈칸 글자는 건너뛴다 |
| 2×2 행렬이 '그림 라벨' 로 버려진다 | 낱글자뿐이고 관계식이 없으면 `_tex_is_figure_junk` 가 버렸다 | 행·열 환경이 있으면 그림이 아니라 수식이다 |

### 조립 규칙 (보수적으로)

격자로 확정되지 않으면 예전 출력을 그대로 쓴다 — 새로 깨지는 수식을 만들지
않는 것이 방침이다(불확실한 벡터는 배경에 원본으로 남는다).

- 구분자 두 개가 내용을 좌우·세로로 품고, 내용 글자보다 1.45배 이상 커야 한다.
- **두 줄 이상**이어야 격자다 — 한 줄 나열(`\left( a+b \right)`,
  `\left[ x_1, x_2, x_3 \right]`)은 건드리지 않는다(1×n 행 벡터는 남는 한계).
- 두 줄 사이를 **분수선**이 지나면 분수다(`(a+b)/(c+d)` 가 3열 행렬이 되지 않게).
- 열은 '칸 안쪽 간격'과 '칸 사이 간격'의 도약으로 가른다. 닿아 있는 간격
  (<1.0pt)은 커닝으로 보고 제외한다 — 점 세 개(`\cdots`) 사이의 3pt 가 칸
  간격으로 뽑혀 줄임표가 세 칸으로 쪼개지던 것을 막는다. 도약이 여러 개면
  **모든 줄의 칸 수가 같은** 분할을 고른다.
- 열이 하나(열 벡터)이면 칸 크기가 같고 세로로 반듯해야 한다 —
  `X^{a}_{b}` 처럼 첨자만 위아래로 붙은 덩어리를 열 벡터로 오인하지 않게.

### 검증

- `npm run test:matrix` (= `python3 test/import_pdf_matrix_runtime.py`) 검사 24개.
  표본 3쪽 — 물리(파울리 2×2·회전 2×2·줄임표 3×3·열 벡터·식 번호 붙은 4×4),
  컴퓨터 구조(의존 행렬 4×4·첨가 행렬·블록 행렬), pdflatex(CMEX) 확장 글꼴 큰
  괄호 2종. **수정 전 코드(`git stash`)로 돌리면 24개 중 16개가 실패한다** —
  2×2 행렬이 `'\\left[ a c b d \\right]'` 로 나와
  `'\\begin{bmatrix} a & b \\\\ c & d \\end{bmatrix}'` 와 어긋나고(행·열 소실),
  표본 쪽에서는 행렬이 하나도 복원되지 않는다.
- 복원된 수식 17개를 앱과 같은 **KaTeX 0.16.11** 로 실제 렌더(throwOnError)해
  오류 0 을 확인했다.
- 기존 검사 28개(`npm run test:pdf`)는 그대로 통과 — 분수·`cases`·첨자·큰
  연산자·식 번호 `(5.44)` 경로는 변하지 않았다.
- 실제 논문 점검: `python3 scripts/import-pdf-matrix-scan.py paper.pdf …`
  (쪽마다 복원된 행렬·수식·경고를 출력, PDF 는 저장소에 넣지 않는다).

## 14.47.0 — 행렬 복원 후속 (Symbol 글꼴·칸 숫자·가운데 행렬)

검증일: **2026-09-10** · 기준 버전: **14.46.0** · 결과 버전: **14.47.0**

> 14.46.0 으로 사용자 제공 PDF 두 편(수학 문제집·Further Pure 9장)을 점검하며
> 발견한 추락을 고쳤다. 검증 뒤 두 PDF 는 저장소에서 지웠다.

| 증상 | 원인 | 고친 방법 |
|---|---|---|
| Symbol 글꼴 수식(`θ`·`−`·`×`·`⇔`·`{ }` 조각)이 통째로 사라지거나 깨진다 | PUA(F000–F8FF) 코드를 읽지 못했다. 구역 글꼴이 `Symbol` 이어도 CMEX 경로를 탔다 | `_symbol_pua_lookup` — Symbol PUA를 글리프 ID로 풀어 KaTeX 명령으로. 괄호 조각은 `_merge_piece_columns` 으로 세로로 이어 붙인다 |
| `-19` 가 `- 1 9` 로, `h_{11}` 이 `h _{1 1}` 로 흩어진다 | 칸 안 자릿수·부호가 낱상자라 간격 계산을 어지럽혔다 | `_micro_merge_row` — 격자 판정 전에 칸 안 부호·자릿수·소수점을 합친다. 크기가 12% 다르면(첨자) 합치지 않는다 |
| 열 간격이 고른 행렬(`1 -1 0 …`)이 통째로 놓친다 | 열 경계 '도약'이 없어 `(0,0)` 후보가 부호를 별도 열로 갈랐다 | 도약이 없으면 최소 간격의 절반을 후보로 둔다(지터에 2배 여유) |
| 행렬 사이 낀 `[x;y]` 두 글자 벡터가 `\\left[ y x \\right]` 로 떨어진다 | `_try_matrix` 의 '상자 4개 미만 거부'가 내용물만 받는 opener/closer 경로까지 막았다 | 그 검사를 자동 탐색 경로에만 적용한다 |
| 한 줄에 행렬이 셋이면 가운데(B)가 `\\left[` 로 떨어진다 | 닫는 괄호 '바로 뒤 첨자' 검사가 **줄 끝까지** 봐서 옆 행렬 윗줄에 쌍이 죽었다 | 바로 뒤(5pt 안팎)만 본다. 덤으로 내용물에 큰 괄호가 또 있으면 걸친 엉터리 쌍으로 보고 건너뛴다(첨가 행렬 막대 제외) |
| 분수 칸이 든 2×2(`[5 1/2; 20 2]`)가 뭉개진다 | 위 가운데-행렬 추락과 같은 원인(쌍이 죽어 분수 분할이 `(2,2)` 칸을 분모로 삼켰다) | 같은 수정으로 `\\begin{bmatrix} 5 & \\frac{1}{2} \\\\ 20 & 2 \\end{bmatrix}` 복원 |

남는 한계(인라인 조립, 행렬 밖): 행렬 밖 낱말(`A ^{- 1}`, `( ^{ad bc} _{-} )`,
분모 `2 2`)의 미세 병합은 손대지 않았다 — 렌더는 되나 띄어쓰기가 느슨하다.

### 검증

- `npm run test:matrix` 검사 **28개**(14.46.0의 24개 + 회귀 4개: 부호 3×3·
  가운데 2글자 벡터·분수 칸 가운데 행렬·실측 기하 자릿수 병합).
  **수정 전 코드로 돌리면 새 검사 4개가 모두 실패한다.**
- 사용자 제공 2편 스캔: Further Pure 9장 **0→174개**, 문제집 **0→6개** 행렬
  복원. KaTeX 거부 경고 0, 흩어진-칸 경고는 62→50개로 감소(인라인 잔재만 남음).
- 기존 검사(`test:pdf` 28개·방향 9개)는 그대로 통과.
