...
>
> **⚠️ 2026-09 v3.1 (14.18.1) — "한 시간 뒤 사라짐"의 실제 서버 원인 발견·수정.**
> v3 로 클라이언트 구조를 단순·확실하게 바꾼 뒤에도 *업로드 직후엔 보이다가
> 잠시 뒤(약 1시간+) 다른 기기에서 404 로 깨지는* 증상이 재발할 수 있는
> **서버 측 킬러**가 따로 있었다: 워커의 음악 백필 스레드가 15분마다 부르는
> `_cleanup_old_temp_files()`(worker/sdynotes_worker/common.py)가
> `imported/`(IMG_DIR) 와 `imported_docs/`(DOCS_DIR) 를 **"mtime 1시간 초과 =
> 임시 파일"**로 보고 통째로 지우고 있었던 것. oracle 자체 저장소 전환(14.12)
> 이후 그 디렉터리들은 노트 사진(`img_*.webp`, `/api/img/`)과 문서 가져오기
> 배경·대용량 문서 본문의 **영구 저장소**인데, 그 사실이 청소 함수에 반영되지
> 않았다. 14.18.1 에서 청소 대상을 "이름으로 확실히 임시인 파일
> (`chunk_*.json`·`*.tmp`·`*.part`, 24시간 넘은 업로드 `*.bin`)만"으로 좁혀
> 영구 데이터를 절대 건드리지 않게 고쳤다. 회귀 방지:
> `python3 test/worker_img_cleanup_contract.py`(`npm run test:imgstore`,
> `test:image` 그룹에도 포함).
