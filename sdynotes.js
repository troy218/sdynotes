...
/* === script block 3 === */
        ...
        (page.els||[]).forEach(el=>{
            if(el.type==='latex'){
                ...
                html+=`<div style="position:absolute;left:${el.x}px;top:${el.y}px;width:${bw}px;height:${bh}px;`+
                      `z-index:5;display:flex;align-items:${imp?'center':(el.displayMath?'center':'flex-end')};${el.displayMath?'justify-content:center;':''}`+
    ...
    function buildStrokeEl(el,pageIdx){
        ...
        if(el.dx||el.dy) g.setAttribute('transform',`translate(${el.dx||0},${el.dy||0})`);
        return g;
    }
...
`;
    ...
    async function pasteFromClipboard(){
        try{
            ...
            if(!t||!t.trim()){ toast('클립보드가 비어 있습니다',1800); return; }
            document.getElementById('cdPasteText').value=t;
            previewCards();
        }catch(e){ toast('붙여넣기 권한이 없습니다 · Ctrl+V 로 넣어 주세요',2600); }
    }
            ...
            try{
                ...
                if(d&&d.ok){
                    ...
                    info.innerHTML=`<b>${d.count}장</b>으로 인식했어요${mix}`+
                        (d.title? ` · 묶음 <b>${esc(d.title)}</b>`:'')+
                        (ex?`<br><span>예: ${esc(ex.front)} → ${esc(ex.back)}</span>`:'');
                }else{
    ...
    async function makeCardsFromText(){
        const t=document.getElementById('cdPasteText').value;
        if(!t.trim()){ toast('내용을 붙여넣어 주세요',1800); return; }
        ...
        try{
            ...
            const d=await r.json();
            if(!r.ok||!d.ok){ toast('실패: '+(d.error||'알 수 없는 오류'),4200); return; }
            toast(`${d.count}장을 만들었습니다`,2200);
    ...
    function copyPrompt(){
        const t=CARD_PROMPT;
        const done=()=>toast('복사했습니다 · AI에 본문과 원하는 문제 수·난이도를 함께 알려 주세요',3200);
        if(navigator.clipboard&&navigator.clipboard.writeText)
            navigator.clipboard.writeText(t).then(done).catch(()=>{fallbackCopyText(t);done();});
        else{ fallbackCopyText(t); done(); }
    }
    // ══════════════════════════════════════════════════════════
    //  11.1 · 암기 카드 정리
    ...
    function renderDeckPage(page){
        ...
        }).join('') : `<div class="cd-empty">‘${esc(q)}’ 과(와) 맞는 묶음이 없어요</div>`;
    ...
    async function startStudy(id){
        try{
            ...
            showPane('cardsStats');
            renderStudyPick();
        }catch(e){ toast('묶음을 열 수 없습니다',2200); }
    }
    function studyPool(){
        ...
        if(hard.length){
            hl.innerHTML='<div class="ana-title" style="margin:4px 0 2px;">자주 틀린 문제 <span style="font-weight:400;color:var(--text3)">눌러서 바로 복습</span></div>'+
                hard.map(c=>`<div class="h" onclick="reviewOne('${c.id}')" style="cursor:pointer">`+
                    `<span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;">${esc(c.front)}</span>`+
    ...
    let _brPage=0;
    function openBrowse(){
        if(!_deckCur){ toast('묶음을 먼저 고르세요',1800); return; }
    ...
    function renderBrowse(page){
        ...
        box.innerHTML=list.length? list.slice(start,start+CARDS_PER_PAGE).map(c=>{
            ...
            const chips=[];
            if(c.tag) chips.push(`<em>#${esc(c.tag)}</em>`);
            if(c.difficulty) chips.push(`<em>${esc(c.difficulty)}</em>`);
            ...
            return `<div class="cd-br" data-card="${c.id}" title="눌러서 이 문제만 풀기">
                ...
                <div class="q"><b>${esc(c.front||'(빈 문제)')}</b><div class="cs">${chips.join('')}</div></div>
            ...
            try{
                if(a==='reset'){
                    ...
                    const r=await fetch('/api/cards/card/delete',{method:'POST',headers:{'Content-Type':'application/json'},
                        body:JSON.stringify({deck:_deckCur.id,card:cid})});
                    const d=await r.json().catch(()=>({}));
                    if(!r.ok||!d.ok){ toast('삭제하지 못했습니다',2000); return; }
                    const i=_deckCur.cards.indexOf(c); if(i>=0) _deckCur.cards.splice(i,1);
                    toast('문제를 삭제했어요',1600);
                }
            }catch(e){ toast('처리하지 못했습니다',2000); return; }
            renderBrowse();
        });
    }
    ...
    async function openCardAnalytics(){
        ...
        try{
            ...
            const groups=(title,arr)=>!arr||!arr.length?'':`<div class="ana-sec"><div class="ana-title">${title}</div>`+
                arr.slice(0,8).map(x=>`<div class="ana-row"><span>${esc(x.name)}</span><div class="track"><i style="width:${x.accuracy}%"></i></div><em>${x.accuracy}% · ${x.attempts}</em></div>`).join('')+'</div>';
            body.innerHTML=`
              <div class="ana-kpis">
              ...
              <div class="ana-sec"><div class="ana-title">다시 보면 좋은 문제</div>
                ${(s.weak||[]).length?(s.weak||[]).map(x=>{const cc=(_deckCur.cards||[]).find(z=>z.id===x.id||z.front===x.front);
                    ...
                    `<span>${esc(x.front)}</span><b>오답 ${x.wrong} · ${x.accuracy}%${cc?' <i class="ri-arrow-right-s-line"></i>':''}</b></div>`;}).join(''):'<div style="font-size:12px;color:var(--text3)">아직 뚜렷한 약점이 없어요. 잘하고 있습니다 ✨</div>'}
              </div>`;
    ...
    function beginStudy(mode,opt){
        ...
        if(!pool.length){ toast('풀 수 있는 문제가 없습니다. 새 묶음은 M(객관식) 형식으로 만들어 주세요',2600); return; }
        let q;
        ...
        if(mode==='quiz'){ q=pool.slice(); shuffle(q); }
            ...
            if(!q.length){ toast('아직 틀린 문제가 없어요 · 먼저 한 바퀴 풀어 볼까요?',2400); return; }
        }
        else if(mode==='cram'){
            ...
            q=(opt&&opt.cards)||[];
            if(!q.length){ toast('다시 풀 문제가 없어요',1800); return; }
            mode='hard';
        }
        else{  // smart
    ...
    function startMatch(pool){
        ...
        if(cand.length<3){ toast('짝 맞추기는 서로 다른 정답이 3개 이상 필요해요',2600); return; }
        ...
        grid.innerHTML=tiles.map((t,i)=>
            ...
            `<span>${esc(t.text)}</span></button>`).join('');
        ...
        if(c.type==='choice'){
            ...
            document.getElementById('cdOpts').innerHTML=order.map((oi,k)=>
                `<button class="cd-opt" data-i="${oi}" onclick="pickOpt(${oi},this)">
                   <i class="num">${k+1}</i><span>${esc(c.opts[oi])}</span></button>`).join('');
            return;
        }
    ...
    function retryWrong(){
        ...
        if(!cards.length){ toast('다시 풀 문제가 없어요',1800); return; }
        shuffle(cards);
    ...
    function renderNotifications(){
        ...
        list.innerHTML=_notifItems.map(n=>`<div class="notif-item ${n.read?'':'unread'}" data-kind="${esc(n.kind||'info')}">
          <div class="notif-ico"><i class="${notifIcon(n.kind)}"></i></div>
          <div class="notif-copy"><b>${esc(n.title||'알림')}</b>${n.message?`<p>${esc(n.message)}</p>`:''}<time>${notifAgo(n.ts)}</time></div>
    ...
    async function notifPoll(){
        ...
        try{
            const ping=fetch('/api/presence/ping',{method:'POST',headers:{'Content-Type':'application/json'},
                body:JSON.stringify({device:SET_DEV})}).then(r=>r.json()).catch(()=>null);
    ...
    document.addEventListener('visibilitychange',()=>{ if(!document.hidden){ notifPoll(); flushCardGrades(); pullSettings(); } });
    ...
    function setShape(m){
        shapeMode=m; eraserActive=false;
        document.getElementById('eraserBtn').classList.remove('active');
    ...
    async function uploadOne(file){
        // 14.12 · 이미지는 항상 이 서버(/api/upload → 오라클 디스크)에 올린다.
        // 예전의 Cloudinary 직접 업로드 폴백은 삭제됐다 (쿼터 초과 방지).
        try{
            const fd=new FormData();
    ...
    // 문서 사전(doc.glossary)에 새 용어 번역을 학습해 저장 → 이후 번역이 일관됨
    async function learnGlossary(texts,target){
        if(!doc) return 0;
        const g=doc.glossary||(doc.glossary={});
    ...
    async function translateDocAction(target){
        ...
        if(doc.__ref&&(doc.pages||[]).some(p=>p.__lazy!=null)){
    ...
    async function reloadImportedIfNewer(){
        ...
        if(!d||!d.__ref||trBusy||_impReloading) return 0;
        _impReloading=true;
        try{
    ...
    async function liveDocTick(){
        ...
        if(document.hidden||document.querySelector('#pagesStage .tb.edit')||trBusy){
    ...
    function _selectiveRenderPage(idx, activeId){
        ...
        els.forEach(el=>{
            ...
            if(domEl){
                ...
                else if(el.type==='stroke'&&svg) svg.appendChild(buildStrokeEl(el,idx));
        ...
        for(const el of page.els||[]){
            if(el.type==='image'&&el.url){
                try{
                    ...
                    ctx.drawImage(im,el.x,el.y,el.w,el.h);
    ...
    async function _boot(){
        ...
        try{ srvStart(); }catch(e){}          // 서버 상태 계기판 시작
    }
